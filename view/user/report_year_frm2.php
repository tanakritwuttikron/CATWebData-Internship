...
<?php
 ...
 if ($_POST["txtnum_time"] != "all") {
   ...
   $sql1="SELECT * FROM data_daily_3 dd JOIN member mb ON dd.id_member = mb.id_member WHERE dd.time_work = '".$strnum_time."' AND YEAR(dd.date_daily) = '".$strnum_year."'";
   $query1 = mysqli_query($conn,$sql1);

   $sql2="SELECT * FROM data_daily_3 dd JOIN member mb ON dd.id_member = mb.id_member WHERE dd.time_work = '".$strnum_time."' AND YEAR(dd.date_daily) = '".$strnum_year."' GROUP BY dd.date_daily,dd.time_work Having count(dd.date_daily) >= 1 ORDER BY dd.date_daily ASC ";
   $query2 = mysqli_query($conn,$sql2);
   $result2 = mysqli_fetch_array($query2);

   $sql3 = "SELECT * FROM data_daily_3 dd JOIN member mb ON dd.id_member = mb.id_member WHERE dd.time_work = '".$strnum_time."' AND YEAR(dd.date_daily) = '".$strnum_year."' GROUP BY dd.time_work Having count(dd.time_work) >= 1 ORDER BY dd.time_work ASC";
  ...
  $sql1="SELECT * FROM data_daily_3 dd JOIN member mb ON dd.id_member = mb.id_member WHERE YEAR(dd.date_daily) = '".$strnum_year."' ";
  $query1 = mysqli_query($conn,$sql1);

   $sql2="SELECT * FROM data_daily_3 dd JOIN member mb ON dd.id_member = mb.id_member WHERE YEAR(dd.date_daily) = '".$strnum_year."' GROUP BY dd.date_daily,dd.time_work Having count(dd.date_daily) >= 1 ORDER BY dd.date_daily ASC ";
   $query2 = mysqli_query($conn,$sql2);
   $result2 = mysqli_fetch_array($query2);

   $sql3 = "SELECT * FROM data_daily_3 dd JOIN member mb ON dd.id_member = mb.id_member WHERE YEAR(dd.date_daily) = '".$strnum_year."' GROUP BY dd.time_work Having count(dd.time_work) >= 1 ORDER BY dd.time_work ASC";
  $query3 = mysqli_query($conn,$sql3);
   ...
   ?>
    <tbody>
      ...
      <td style="border-color: #ffffff;">
        <button style="margin-bottom: 5px;" type="button" class="btn edit-daily-y-3" id="btn-edit" data-toggle="modal" data-target="#editReport3YearModal"
    ...
    <?php require '../modal/md_edit_daily_y_3.php'; ?>
  <?php } ?>
   </table>
  </div>
  </div><!-- ROW 2 -->
